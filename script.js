...
// Arquivo: script.js (VERSÃO 8.0 - Adiciona menu lateral responsivo)
...
/**
* Interpreta a mensagem do usuário e decide qual ação tomar (GET ou POST).
* (VERSÃO 8.0 - Mantém lógica da 7.9)
*/
async function processUserMessage(message) {
    // (Seu código processUserMessage aqui... sem mudanças)
    // ...
    // ... (cole sua função processUserMessage inteira aqui) ...
    // ...
    const cleanedMessage = message.trim().replace(/[.!?]$/, '');
    const lowerMessage = cleanedMessage.toLowerCase();
    let match;
 ...
 */
async function sendDataToAPI(data) {
    // (Seu código sendDataToAPI aqui... sem mudanças)
    // ...
    if (typeof API_URL === 'undefined') {
    ...
    try {
        ...
        throw new Error(`A API (POST) retornou uma resposta inesperada: ${responseText.substring(0, 100)}...`);
    }
}
...
async function getDataFromAPI(filtro, params = {}, action = 'consultar') {
    // (Seu código getDataFromAPI aqui... sem mudanças)
    // ...
    if (typeof API_URL === 'undefined') {
        throw new Error("API_URL não está definida.");
    }
    ...
    try {
        ...
        throw new Error(`A API (GET) retornou uma resposta inesperada: ${responseText.substring(0, 100)}...`);
    }
}
